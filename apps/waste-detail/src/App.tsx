                ...
                <main className="flex-1 overflow-y-auto">
                    ...
                    <div className="p-4 flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="font-extrabold text-lg tracking-tight text-slate-900 dark:text-slate-100">Kerabat Kopi Tiam</p>
                            <div className="flex flex-wrap items-center gap-2 mt-2">
                                <span className="px-2.5 py-1 rounded-md text-[10px] font-extrabold bg-primary/10 dark:bg-primary/20 text-primary border border-primary/20 dark:border-primary/30 uppercase tracking-wider shadow-sm">{item.category}</span>
                    ...
                    <div className="px-4 grid grid-cols-2 gap-3 pb-2 pt-2">
                        <div className="col-span-2 bg-slate-50 dark:bg-primary/5 border border-slate-200 dark:border-primary/10 rounded-2xl p-5 shadow-sm">
                            ...
                            <div className="flex items-end justify-between mt-2">
                                <p className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">
                                    {totalTerbuang} <span className="text-lg font-semibold text-slate-500 dark:text-slate-400 ml-1">{item.unit}</span>
                                </p>
                        ...
                        <div className="bg-slate-50 dark:bg-primary/5 border border-slate-200 dark:border-primary/10 rounded-2xl p-4 shadow-sm">
                            ...
                            <p className="text-lg font-extrabold mt-1.5 tracking-tight text-slate-900 dark:text-slate-100">Rp {nilaiKerugian.toLocaleString('id-ID')}</p>
                        </div>

                        <div className="bg-slate-50 dark:bg-primary/5 border border-slate-200 dark:border-primary/10 rounded-2xl p-4 shadow-sm">
                            <p className="text-slate-500 dark:text-slate-400 text-[10px] font-bold uppercase tracking-wider">Harga Satuan</p>
                            <p className="text-lg font-extrabold mt-1.5 tracking-tight text-slate-900 dark:text-slate-100">Rp {parseFloat(item.pricePerUnit).toLocaleString('id-ID')}</p>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            {wasteLogs.length === 0 ? (
                                ...
                                wasteLogs.map((log: any) => (
                                    <div key={log.id} className="bg-white dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="flex justify-between items-start">
                                            <div className="flex flex-col gap-0.5">
                                                <p className="font-extrabold text-sm tracking-tight text-slate-900 dark:text-slate-100">{log.quantity} {item.unit}</p>
                                                <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400">{log.reason || 'Tidak ada alasan'}</p>
                                            </div>
