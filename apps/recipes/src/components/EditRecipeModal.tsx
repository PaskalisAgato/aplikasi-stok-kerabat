...
export default function EditRecipeModal({ recipe, onClose, onSave }: EditRecipeModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] dark:bg-black bg-white/40 backdrop-blur-md flex justify-center items-center sm:p-4 animate-in fade-in duration-300">
            <div className="relative flex h-full max-h-screen sm:max-h-[90vh] sm:min-h-[500px] w-full flex-col max-w-md mx-auto shadow-2xl bg-[var(--bg-app)] sm:rounded-[2.5rem] border-x sm:border border-[var(--border-dim)] overflow-hidden animate-in zoom-in-95 duration-300">
                {/* Top App Bar */}
                <header className="sticky top-0 z-50 bg-[var(--bg-app)] border-b dark:border-white/5 border-slate-200 px-4 py-4 flex items-center gap-2 backdrop-blur-md bg-opacity-95">
                    <div className="flex-1 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                <span className="material-symbols-outlined dark:text-white dark:text-white text-slate-900">arrow_back</span>
                            </button>
                            <h1 className="text-lg font-bold tracking-tight dark:text-white dark:text-white text-slate-900 truncate w-[200px]">
                ...
                <main className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="px-4 pt-6">
                        <label className="block text-[10px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Nama Resep</label>
                        <input
                            type="text"
                            value={namaResep}
                            onChange={(e) => setNamaResep(e.target.value)}
                            placeholder="Masukkan nama resep..."
                            className="w-full glass dark:border-white/5 border-slate-200 rounded-2xl py-4 px-5 text-lg font-extrabold focus:ring-4 focus:ring-primary/10 transition-all shadow-sm dark:text-white dark:text-white text-slate-900 placeholder:dark:text-slate-400 dark:text-slate-400 text-slate-500"
                        />
                    </div>

                    {/* Image Upload UI */}
                    <div className="px-4 pt-6">
                        <label className="block text-[10px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-[0.2em] mb-1.5 ml-1">Foto Menu</label>
                        <div className="flex gap-4 items-center">
                            ...
                            >
                                {imageUrl ? (
                                    <>
                                        ...
                                        <div className="absolute inset-0 dark:bg-black bg-white/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                            ...
                            <div className="flex-1 space-y-1">
                                <p className="text-[10px] font-black dark:text-white dark:text-white text-slate-900 uppercase tracking-widest">
                                    {imageUrl ? 'Ubah Foto' : 'Tambah Foto'}
                                </p>
                                <p className="text-[8px] font-bold dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-widest opacity-60">
                    ...
                    {/* Category Selection */}
                    <div className="px-4 pt-6">
                        <label className="block text-[10px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1 text-center">Kategori Menu</label>
                            ...
                            {['Minuman', 'Makanan', 'Snack'].map(cat => (
                                <button
                                    ...
                                    className={`px-6 py-2.5 rounded-full text-xs font-black transition-all active:scale-90 border-2 ${
                                        ...
                                        : 'glass dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:border-white/5 border-slate-200 hover:border-primary/30'
                                    }`}
                                >
                                    {cat}
                                </button>
                    ...
                    <section className="p-4">
                        <div className="flex items-center justify-between mb-4 relative z-20">
                            ...
                            <div className="relative">
                                ...
                                {showAddIngredient && (
                                    <div className="absolute right-0 top-full mt-3 w-80 glass rounded-[2rem] shadow-2xl dark:border-white/10 border-slate-200 overflow-hidden transform origin-top-right animate-in slide-in-from-top-4 fade-in duration-300 z-[60]">
                                        <div className="p-4 border-b dark:border-white/5 border-slate-200 dark:bg-white/5 bg-white shadow-sm border border-slate-200">
                                            <div className="relative">
                                                ...
                                                <input
                                                    ...
                                                    className="w-full dark:bg-white/5 bg-white shadow-sm border border-slate-200 border-none rounded-xl pl-10 pr-4 py-2.5 text-sm font-bold focus:ring-2 focus:ring-primary shadow-inner dark:text-white dark:text-white text-slate-900 placeholder:dark:text-slate-400 dark:text-slate-400 text-slate-500"
                                                    autoFocus
                                                />
                                            </div>
                                        </div>
                                        <div className="max-h-[350px] overflow-y-auto scrollbar-none">
                                            {isSearching ? (
                                                ...
                                                <div className="p-8 text-center opacity-40">
                                                    <span className="material-symbols-outlined text-3xl mb-1 dark:text-slate-400 dark:text-slate-400 text-slate-500">keyboard</span>
                                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] dark:text-slate-400 dark:text-slate-400 text-slate-500">Ketik Minimal 2 Karakter</p>
                                                </div>
                                            ) : searchTerm && availableIngredients.length === 0 ? (
                                                <div className="p-10 text-center opacity-40">
                                                    <span className="material-symbols-outlined text-3xl mb-1 dark:text-slate-400 dark:text-slate-400 text-slate-500">sentiment_dissatisfied</span>
                                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] dark:text-slate-400 dark:text-slate-400 text-slate-500">Bahan Tidak Ditemukan</p>
                                                </div>
                                            ) : (
                                                <div className="py-2">
                                                    {!searchTerm && (
                                                    ...
                                                    {availableIngredients.map(item => (
                                                        ...
                                                        >
                                                            <div className="space-y-0.5">
                                                                <p className="font-black text-sm dark:text-white dark:text-white text-slate-900 group-hover:text-primary transition-colors">{item.name}</p>
                                                                <p className="text-[9px] dark:text-slate-400 dark:text-slate-400 text-slate-500 font-bold uppercase tracking-wider opacity-60">{item.category} • {formatRp(parseFloat(item.pricePerUnit))}/{item.unit}</p>
                                                            </div>
                        ...
                        <div className="space-y-3 relative z-10">
                            {ingredients.map(ing => (
                                <div key={ing.id} className="glass rounded-2xl p-4 shadow-sm dark:border-white/5 border-slate-200">
                                    <div className="flex justify-between items-start mb-3.5">
                                        <div>
                                            <p className="font-bold text-[15px] dark:text-white dark:text-white text-slate-900 tracking-tight">{ing.name}</p>
                                            <div className="flex items-center gap-1 mt-1">
                                                <p className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 font-bold uppercase tracking-wider">Harga Unit: Rp </p>
                                                <input 
                                                    type="number"
                                                ...
                                                />
                                                <p className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 font-bold uppercase tracking-wider"> / {ing.unit}</p>
                                            </div>
                                        </div>
                                        <button
                                    ...
                                    <div className="mb-4 p-3 dark:bg-white/5 bg-white shadow-sm border border-slate-200 rounded-xl border border-primary/10">
                                        <p className="text-[9px] font-black text-primary uppercase tracking-[0.1em] mb-2">Kalkulator Konversi Harga Beli</p>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="space-y-1">
                                                <label className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 font-bold ml-1">Harga Beli (Rp)</label>
                                                <input
                                                    type="number"
                                                    ...
                                                    className="w-full dark:bg-white/5 bg-white shadow-sm border border-slate-200 border dark:border-white/10 border-slate-200 rounded-lg py-1.5 px-3 text-sm font-bold dark:text-white dark:text-white text-slate-900 focus:ring-1 focus:ring-primary"
                                                />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 font-bold ml-1">Total Berat/Vol ({ing.unit})</label>
                                                <input
                                                    type="number"
                                        ...
                                        <p className="text-[9px] dark:text-slate-400 dark:text-slate-400 text-slate-500 mt-2 italic">* Input 1000 jika beli per 1kg/1L untuk mendapatkan harga per g/ml</p>
                                    </div>

                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 dark:bg-white/5 bg-white shadow-sm border border-slate-200 rounded-xl p-1 border dark:border-white/5 border-slate-200 shadow-inner">
                                            <button
                                            ...
                                            <div className="flex items-center gap-1">
                                                <input
                                                    className="w-12 text-center bg-transparent border-none focus:ring-0 p-0 font-extrabold text-base dark:text-white dark:text-white text-slate-900"
                                                    type="number"
                                        ...
                                        <div className="text-right">
                                            <p className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase font-bold tracking-wider">Subtotal</p>
                        ...
                        {/* Overhead & Buffer */}
                        <div className="mt-6 border-t dark:border-white/10 border-slate-200 pt-5">
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-2">
                                    <span className="material-symbols-outlined text-primary text-[18px]" style={{ fontVariationSettings: "'FILL' 1" }}>bolt</span>
                                    <h2 className="text-sm font-bold tracking-tight dark:text-white dark:text-white text-slate-900">Overhead & Buffer (%)</h2>
                                </div>
                            ...
                            <div className="relative">
                                <input
                                    className="w-full glass dark:border-white/5 border-slate-200 rounded-xl py-3.5 px-4 focus:ring-2 focus:ring-primary/40 transition-colors pr-10 font-bold dark:text-white dark:text-white text-slate-900 shadow-sm"
                                    ...
                                    min={0}
                                />
                                <span className="absolute right-4 top-1/2 -translate-y-1/2 dark:text-slate-400 dark:text-slate-400 text-slate-500 font-extrabold text-lg">%</span>
                            </div>
                            <p className="text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 mt-2 px-1 font-medium leading-relaxed">
                    ...
                    <section className="p-4 mt-2 mb-10">
                        <h2 className="text-base font-bold mb-4 border-l-4 border-primary pl-3 dark:text-white dark:text-white text-slate-900 tracking-tight text-shadow">Analisa Margin</h2>

                        <div className="glass rounded-2xl p-5 shadow-sm dark:border-white/5 border-slate-200">
                            <div className="space-y-4">
                                {/* Selling Price */}
                                <div className="flex justify-between items-center">
                                    <span className="text-sm font-medium dark:text-slate-400 dark:text-slate-400 text-slate-500">Harga Jual per Cup</span>
                                    <div className="flex items-center gap-1.5 border-b-2 border-primary/30 pb-1">
                                        <span className="text-xs font-extrabold dark:text-slate-400 dark:text-slate-400 text-slate-500">Rp</span>
                                        <input
                                            className="bg-transparent border-none p-0 text-right font-extrabold text-lg focus:ring-0 w-24 dark:text-white dark:text-white text-slate-900"
                                            type="text"
                                ...
                                <div className="flex justify-between items-center text-sm">
                                    <span className="font-medium dark:text-slate-400 dark:text-slate-400 text-slate-500">Total HPP (Bahan + Overhead)</span>
                                    <span className="font-bold dark:text-white dark:text-white text-slate-900">{formatRp(totalHPP)}</span>
                                </div>
                                <hr className="border-dashed dark:border-white/10 border-slate-200" />
                                ...
                                <div className="flex justify-between items-center py-2.5 px-4 bg-primary/10 rounded-xl border border-primary/20">
                                    ...
                                    <div className="text-right">
                                        <p className="text-[9px] dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase font-extrabold tracking-widest mb-0.5">Laba bersih</p>
                                        <p className="text-xl font-extrabold dark:text-white dark:text-white text-slate-900 tracking-tight">{formatRp(laba)}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {/* Bottom Actions */}
                <footer className="glass border-t dark:border-white/10 border-slate-200 p-5 pb-10 z-50 max-w-md mx-auto shadow-[0_-15px_40px_rgba(0,0,0,0.1)] w-full">
                    <div className="flex gap-4">
                        <button
                            onClick={onClose}
                            className="flex-[1] py-5 glass hover:dark:bg-white/10 bg-white shadow-md border border-slate-200 dark:text-slate-400 dark:text-slate-400 text-slate-500 font-black text-[10px] uppercase tracking-[0.2em] rounded-[2rem] transition-all active:scale-95 dark:border-white/5 border-slate-200">
                            Batal
                        </button>
                        <button
                            onClick={handleSaveAPI}
                            disabled={isUploadingImage}
                            className={`flex-[2.5] py-5 ${isUploadingImage ? 'bg-slate-400' : 'bg-gradient-to-r from-primary to-amber-600'} dark:text-white dark:text-white text-slate-900 font-black text-[10px] uppercase tracking-[0.2em] rounded-[2rem] shadow-xl shadow-primary/30 transition-all flex items-center justify-center gap-3 active:scale-95 group`}>
