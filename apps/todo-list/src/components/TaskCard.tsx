        ...
        <div className={`card group p-5 flex flex-col gap-4 transition-all border-white/5 relative ${isCompleted ? 'opacity-60 grayscale-[0.5]' : ''} ${isOverdue ? 'ring-2 ring-red-500/50 bg-red-500/5 shadow-[0_0_20px_rgba(239,68,68,0.1)]' : ''}`}>
            ...
            {role === 'Karyawan' && (
                <div className="pt-4 border-t border-white/5 flex flex-col gap-3">
                    ...
                    {/* Upload buttons — synced with photoUploadMode setting */}
                    {!isCompleted && (
                        <>
                            {photoUploadMode === 'both' ? (
                                ...
                                <button 
                                    onClick={() => document.getElementById(`gallery-${task.id}`)?.click()}
                                    disabled={isUploading}
                                    className={`w-full h-12 rounded-xl flex items-center justify-center gap-3 transition-all border-2 border-dashed ${isUploading ? 'bg-emerald-500/5 border-emerald-500/40 animate-pulse' : 'bg-white/5 border-white/10 hover:border-emerald-500/40 hover:bg-emerald-500/5'}`}
                                >
                                    <span className="material-symbols-outlined text-emerald-500 text-xl font-black">
                                        {isUploading ? 'hourglass_empty' : 'photo_library'}
                                    </span>
                                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">
                                        {isUploading ? 'Mengunggah...' : 'PILIH DARI GALERI'}
                                    </span>
                                </button>
                            ) : (
                                <button 
                                ...
                                >
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest text-primary">
                                        {isUploading ? 'Mengunggah...' : 'AMBIL FOTO KAMERA'}
                                    </span>
                                </button>
                            )}
                        </>
