...
function App() {
    ...
    useEffect(() => {
        const fetchCOGS = async () => {
            try {
                const response = await apiClient.getCOGSAnalysis();
                setCogsData(response.data);
            } catch (error) {
                console.error('Failed to fetch COGS analysis', error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const topItem = cogsData?.topExpenditureItems?.[0];
    const totalProcurement = safeNumber(cogsData?.totalProcurement);
    const trend = safeNumber(cogsData?.trendPercentage);
        ...
        >
            {isLoading ? (
                <div className="flex flex-col justify-center items-center py-32 gap-6 glass rounded-[3rem] opacity-60">
                    ...
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] animate-pulse">Memuat Data Pengadaan...</p>
                </div>
            ) : (
                        ...
                        <div className="card group p-8 flex flex-col justify-between hover:scale-[1.02] transition-transform duration-500 border-white/5 relative overflow-hidden">
                            ...
                            <div className="mt-8 pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className={`text-[9px] font-black uppercase tracking-widest ${isTrendUp ? 'text-red-500' : 'text-emerald-500'}`}>
                                        {isTrendUp ? '+' : ''}{trend.toFixed(1)}%
                                    </span>
                                </div>
                            </div>
                             {/* Decorative Background Blob */}
