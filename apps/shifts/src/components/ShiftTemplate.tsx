...
const SHIFT_TYPES = [
    ...
    { label: 'Malam', code: 'M', codeHex: '#94a3b8', color: 'bg-slate-500/20 dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 border-slate-500/50', activeColor: 'bg-slate-500 dark:text-white dark:text-white text-slate-900' },
...
export default function ShiftTemplate({ employees: initialEmployees, allShifts: initialShifts, isLoading }: ShiftTemplateProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-1000 pb-32">
                ...
                <div className="space-y-6">
                    <div className="glass p-6 rounded-[2.5rem] border-primary/20 bg-primary/5">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs font-bold dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500">Periode Aktif • {startDate} - {endDate}</p>
                            </div>
                        </div>
                        <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide">
                            {dates.map(date => (
                                <div key={date} className={`min-w-[110px] p-5 rounded-2xl border flex flex-col items-center gap-3 transition-all ${myShifts[date] && myShifts[date] !== 'OFF' ? 'bg-primary text-slate-950 border-primary scale-105 shadow-xl shadow-primary/20' : 'bg-gray-100/50 dark:bg-white/5 border-gray-200 dark:border-white/5 opacity-40'}`}>
                                    <p className="text-xs font-black uppercase opacity-60 text-gray-600 dark:text-slate-400 dark:text-slate-400 text-slate-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {(['P', 'S', 'M'] as const).map(code => (
                            <div key={code} className={`glass p-5 rounded-3xl border dark:border-white/5 border-slate-200 flex items-center gap-4 transition-all ${shiftSettings[code].active ? '' : 'opacity-40 grayscale'}`}>
                                ...
                                <div>
                                    <h4 className="text-sm font-black uppercase tracking-widest dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 mb-1">Shift {code === 'P' ? 'Pagi' : code === 'S' ? 'Sore' : 'Malam'}</h4>
                                    <p className="text-xs font-bold dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500">{shiftSettings[code].start} - {shiftSettings[code].end}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {isAdmin && (
                <div className="glass p-8 rounded-[2.5rem] dark:border-white/5 border-slate-200 space-y-8">
                    <div className="flex items-center gap-4">
                        <div className="size-12 min-w-[3rem] rounded-2xl bg-gray-100 dark:bg-slate-800 flex items-center justify-center dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900">
                            <span className="material-symbols-outlined text-2xl font-black">settings_accessibility</span>
                        </div>
                        <div className="flex-1 min-w-0">
                            <h2 className="text-lg font-black uppercase tracking-widest dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 leading-tight truncate">Pengaturan Shift</h2>
                            <p className="text-xs font-bold dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-wider truncate">Jam operasional & status shift</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {(['P', 'S', 'M'] as const).map(code => (
                            <div key={code} className={`p-6 rounded-3xl border transition-all duration-500 ${shiftSettings[code].active ? 'bg-white/[0.03] dark:border-white/10 border-slate-200' : 'dark:bg-black bg-white/20 dark:border-white/5 border-slate-200 opacity-40 grayscale'}`}>
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-xs font-black uppercase tracking-widest text-gray-700 dark:text-white dark:text-white text-slate-900">Shift {code === 'P' ? 'Pagi' : code === 'S' ? 'Sore' : 'Malam'}</span>
                                    </div>
                                    <button 
                                    ...
                                    >
                                        <div className={`absolute top-1 size-4 rounded-full dark:bg-slate-950 bg-slate-50 transition-all duration-300 ${shiftSettings[code].active ? 'left-7' : 'left-1'}`} />
                                    </button>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <p className="text-[9px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-widest ml-1">Masuk</p>
                                        <input type="time" value={shiftSettings[code].start} onChange={e => setShiftSettings(p => ({ ...p, [code]: { ...p[code], start: e.target.value } }))} disabled={!shiftSettings[code].active} className="w-full bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-bold dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 outline-none focus:border-primary transition-all disabled:opacity-30" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <p className="text-[9px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 uppercase tracking-widest ml-1">Pulang</p>
                                        <input type="time" value={shiftSettings[code].end} onChange={e => setShiftSettings(p => ({ ...p, [code]: { ...p[code], end: e.target.value } }))} disabled={!shiftSettings[code].active} className="w-full bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-xs font-bold dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 outline-none focus:border-primary transition-all disabled:opacity-30" />
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass p-6 rounded-[2rem] dark:border-white/5 border-slate-200 flex flex-col md:flex-row gap-4 items-center">
                    <div className="flex-1 w-full space-y-2">
                        <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] ml-2">Mulai Dari</label>
                        <input type="date" value={startDate} disabled={!isAdmin} onChange={e => setStartDate(e.target.value)} className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-5 py-3 text-sm font-bold dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 outline-none focus:border-primary transition-all" />
                    </div>
                    <div className="flex-1 w-full space-y-2">
                        <label className="text-[10px] font-black text-primary uppercase tracking-[0.2em] ml-2">Sampai Dengan</label>
                        <input type="date" value={endDate} disabled={!isAdmin} onChange={e => setEndDate(e.target.value)} className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-5 py-3 text-sm font-bold dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 outline-none focus:border-primary transition-all" />
                    </div>
                </div>
                <div className="flex gap-4 items-stretch">
                    <div className="flex glass rounded-[2rem] dark:border-white/5 border-slate-200 p-1">
                        <button 
                            onClick={() => setZoom(Math.max(0.5, zoom - 0.1))}
                            className="p-3 hover:dark:bg-white/5 bg-white shadow-sm border border-slate-200 rounded-2xl text-primary transition-all"
                            title="Zoom Out"
                        >
                        ...
                        <button 
                            onClick={() => setZoom(1)}
                            className="px-4 text-[10px] font-black text-primary uppercase tracking-widest hover:dark:bg-white/5 bg-white shadow-sm border border-slate-200 rounded-2xl transition-all"
                        >
                            {Math.round(zoom * 100)}%
                        </button>
                        <button 
                    ...
                    {isAdmin && (
                        <div className="flex-1 glass p-4 rounded-[2rem] dark:border-white/5 border-slate-200 flex items-center justify-center">
                            <select 
                                className="w-full bg-transparent text-sm font-black uppercase tracking-widest dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-white dark:text-white text-slate-900 outline-none appearance-none text-center cursor-pointer"
                                onChange={e => {
                            ...
                            >
                                <option value="" className="bg-white dark:bg-slate-900 dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900">+ Tambah Karyawan</option>
                                {allEmployees?.filter((u: any) => !gridData.some(g => g.id === u.id)).map((u: any) => (
                                    <option key={u.id} value={u.id} className="bg-white dark:bg-slate-900 dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900">{u.name}</option>
                                ))}
                            </select>
                        </div>
                    )}
                    <button onClick={handleExport} className="glass w-20 rounded-[2rem] dark:border-white/5 border-slate-200 flex items-center justify-center text-primary hover:scale-105 active:scale-95 transition-all shadow-xl shadow-black/20">
            ...
            <div className="relative group/table overflow-hidden rounded-[2.5rem] border dark:border-white/5 border-slate-200 glass shadow-2xl">
                    ...
                    >
                            ...
                            <tr className="bg-gray-100/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-gray-200 dark:border-white/5">
                                ...
                                {dates.map(date => (
                                    <th key={date} className="px-4 py-6 min-w-[100px] text-center border-r border-gray-200 dark:border-white/5">
                                        ...
                                        <p className="text-sm font-black dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900 leading-none">{date.split('-')[2]}/{date.split('-')[1]}</p>
                                    </th>
                                ))}
                        ...
                        <tbody>
                            {gridData.map((row, rIdx) => (
                                <tr key={row.id} className={`border-b border-gray-200 dark:border-white/5 transition-all ${row.id === currentUser?.id ? 'bg-primary/5' : ''}`}>
                                        ...
                                        <div className="flex items-center justify-between gap-3">
                                            <div className="min-w-0">
                                                <div className="flex items-center gap-2">
                                                    <p className={`text-sm font-black uppercase truncate ${row.id === currentUser?.id ? 'text-primary' : 'dark:text-white dark:text-white text-slate-900 dark:text-white dark:text-white text-slate-900'}`}>{row.name}</p>
                                                ...
                                                <p className="text-[10px] font-bold dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500">ID: {row.id.slice(0, 8)}</p>
                                            </div>
                                            {isAdmin && (
                                    ...
                                    <td className="text-center font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 text-xs border-r border-gray-200 dark:border-white/5">{calculations[rIdx].M}</td>
                        ...
                        <tfoot>
                            <tr className="bg-gray-100/50 dark:bg-slate-900/50 backdrop-blur-xl border-t border-gray-200 dark:border-white/5">
                                <td className="p-6 border-r border-gray-200 dark:border-white/5 font-black uppercase tracking-widest text-[10px] dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500 text-center">Rekap Harian</td>
                                {dailyRecap.map((day, i) => (
                                    <td key={i} className="p-4 border-r border-gray-200 dark:border-white/5">
                                        <div className="flex flex-col gap-0.5 items-center">
                                            ...
                                            <span className="text-[10px] font-black dark:text-slate-400 dark:text-slate-400 text-slate-500 dark:text-slate-400 dark:text-slate-400 text-slate-500">M: {day.M}</span>
                                        </div>
                                    </td>
                                ))}
                                <td colSpan={4}></td>
                ...
                {/* Popover Menu */}
                {isAdmin && isMenuOpen && (
                    <div className="fixed z-50 flex gap-4 p-4 glass border dark:border-white/10 border-slate-200 rounded-[2rem] shadow-2xl animate-in zoom-in duration-200" style={{ top: isMenuOpen.y - 100, left: isMenuOpen.x - 120 }}>
