...
function App() {
    ...
    return (
        <Layout 
            ...
            maxWidth="1200px"
        >
            <div className="relative flex min-h-[calc(100vh-200px)] w-full flex-col pb-40 animate-in fade-in slide-in-from-bottom-4 duration-700">
                
                {/* Sales Summary Bar (Premium Glass) */}
                <div className="glass p-8 rounded-[2.5rem] mb-10 transition-all shadow-2xl relative overflow-hidden group">
                ...
                <div className="flex-1 space-y-6">
                    {activeCartItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 text-center glass rounded-[3rem] border-dashed border-2 border-[var(--border-dim)] opacity-60">
                        ...
                        activeCartItems.map(recipe => (
                            <div key={recipe.id} className="card flex items-center gap-6 group hover:scale-[1.01] active:scale-[0.99]">
                                <div
                                    className="size-24 rounded-[1.5rem] bg-cover bg-center shrink-0 shadow-lg border-2 border-white/10 group-hover:rotate-2 transition-transform"
                                ...
                                <div className="flex-1 min-w-0 space-y-1">
                                    <h3 className="font-black text-[var(--text-main)] text-xl font-display tracking-tight leading-tight truncate">{recipe.name}</h3>
                ...
                {/* Footer Action (Floating Glass) */}
                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-[1200px] z-[40]">
                    <div className="glass rounded-[2.5rem] p-4 shadow-2xl shadow-black/40 border-white/20">
                        <button
                            onClick={async () => {
                            ...
                            className={`w-full py-6 rounded-[1.8rem] font-black text-xl tracking-[0.1em] shadow-2xl transition-all active:scale-[0.97] flex items-center justify-center gap-4 uppercase font-display
