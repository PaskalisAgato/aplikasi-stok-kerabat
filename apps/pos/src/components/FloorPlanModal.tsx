...
export const FloorPlanModal: React.FC<FloorPlanModalProps> = ({
    isOpen, onClose, onSelectTable, openBills
}) => {
    ...
    // Calculate max bounds to autoscale the viewport 
            ...
            <div className="bg-white w-full max-w-5xl h-full max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="p-4 md:p-6 bg-slate-50 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
                    <div>
                        ...
                        <p className="text-xs font-bold text-slate-500 mt-1">Tap salah satu meja atau ketik nama untuk pesanan bawa pulang (Takeaway).</p>
                    </div>
                ...
                <div className="flex-1 bg-slate-200/50 p-6 relative overflow-auto custom-scrollbar flex items-center justify-center">
                    {loading ? (
                        ...
                        <div className="text-center opacity-50">
                            ...
                            <p className="font-bold">Denah belum diatur.<br/>Gunakan input garis atas, atau atur di Menu Pengaturan.</p>
                        </div>
                    ) : (
                                ...
                                {tables.map(table => {
                                    ...
                                    return (
                                        <button
                                            key={table.id}
                                            onClick={() => {
                                                if (!isOccupied) onSelectTable(table.name);
                                            }}
                                            disabled={isOccupied}
                                            style={{
                                                position: 'absolute',
                                            ...
                                            className={`
                                                flex flex-col items-center justify-center shadow-md select-none transition-all Group active:scale-95
                                                ${table.shape === 'circle' ? 'rounded-full' : 'rounded-2xl'}
                                                ${isOccupied 
                                                    ? 'bg-red-50 text-red-500 border-2 border-red-200 cursor-not-allowed opacity-80' 
                                                    : 'bg-white text-slate-700 border-2 border-emerald-400 hover:bg-emerald-50 hover:text-emerald-700 hover:border-emerald-500 cursor-pointer hover:shadow-xl hover:shadow-emerald-500/20'
                                                }
                                            `}
                                        >
                                            <span className="font-black text-base truncate px-2 text-center break-words leading-tight">{table.name}</span>
                                            {isOccupied && <span className="text-[10px] uppercase font-bold tracking-widest mt-1 opacity-70 border border-current px-1.5 rounded bg-red-100">Terisi</span>}
                                        </button>
                                    );
                                })}
                            </div>
