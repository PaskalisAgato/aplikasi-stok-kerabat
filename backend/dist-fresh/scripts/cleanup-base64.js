...
async function cleanupBase64Data() {
    ...
    try {
        ...
        const invResult = await db.update(schema.inventory)
            .set({ imageUrl: null })
            .where(and(isNotNull(schema.inventory.externalImageUrl), isNotNull(schema.inventory.imageUrl)))
        ...
        const expResult = await db.update(schema.expenses)
            .set({ receiptUrl: null })
            .where(and(isNotNull(schema.expenses.externalReceiptUrl), isNotNull(schema.expenses.receiptUrl)))
        ...
        const userResult = await db.update(schema.users)
            .set({ image: null })
            .where(and(isNotNull(schema.users.externalImage), isNotNull(schema.users.image)))
