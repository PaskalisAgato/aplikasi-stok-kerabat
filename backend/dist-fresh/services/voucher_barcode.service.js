...
export class VoucherService {
     ...
     */
    static async generateVoucher(transactionId, locationSource) {
     ...
     * Redeem a voucher
     */
    static async redeemVoucher(code, locationRedeemed, transactionId) {
        const { valid, voucher, message } = await this.validateVoucher(code);
        if (!valid || !voucher)
            throw new Error(message || 'Voucher tidak valid');
    ...
    static async getVoucherAnalytics() {
        const total = await db.select({ count: sql `count(*)` }).from(schema.standVouchers);
        const redeemed = await db.select({ count: sql `count(*)` }).from(schema.standVouchers).where(eq(schema.standVouchers.status, 'redeemed'));
        const expired = await db.select({ count: sql `count(*)` }).from(schema.standVouchers).where(eq(schema.standVouchers.status, 'expired'));
