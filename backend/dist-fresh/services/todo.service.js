...
import { todos, todoCompletions, todoSettings } from '../db/schema.js';
import { eq, and, or, isNull, desc, gte, sql } from 'drizzle-orm';
export class TodoService {
    static async getAllTodos(role, userId) {
        // Get Jakarta time (UTC+7) start of day
        const now = new Date();
        const startOfDay = new Date(now.getFullYear(), now.getMonth(), now.getDate());
        // Fetch all relevant todos (Limit 100 for safety)
        const allTodos = await db.query.todos.findMany({
            where: and(eq(todos.status, 'Pending'), or(isNull(todos.nextRunAt), sql `${todos.nextRunAt} <= ${now}`), role === 'Admin' ? undefined : or(isNull(todos.assignedTo), eq(todos.assignedTo, userId || ''))),
    ...
    static async getHistory(limit = 10, offset = 0) {
        ...
        const [onceOffCount] = await db.select({ count: sql `count(*)` })
            .from(todos)
            .where(and(eq(todos.status, 'Completed'), eq(todos.isRecurring, false)));
        const [recurringCount] = await db.select({ count: sql `count(*)` })
    ...
    static async completeTodo(id, userId, photoProof) {
        const todo = await db.query.todos.findFirst({ where: eq(todos.id, id) });
        if (!todo)
            throw new Error('Task not found');
        const serializedPhotoProof = (Array.isArray(photoProof) ? JSON.stringify(photoProof) : photoProof);
        ...
        if (todo.isRecurring) {
            ...
            const existing = await db.query.todoCompletions.findFirst({
                where: and(eq(todoCompletions.todoId, id), gte(todoCompletions.completionTime, startOfDay))
            });
            if (existing)
                return existing;
            ...
            if (todo.isRecurring && todo.intervalType) {
                const nextRun = this.calculateNextRun(todo.nextRunAt || now, todo.intervalType, todo.intervalValue || 1);
    ...
    static calculateNextRun(current, type, value) {
    ...
    static async getCompletionPhoto(id) {
