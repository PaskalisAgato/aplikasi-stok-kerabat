...
export class CashierShiftService {
    static async getActiveShift(userId) {
        const activeShifts = await db.select()
            .from(schema.shifts)
            .where(and(eq(schema.shifts.userId, userId), eq(schema.shifts.status, 'OPEN')))
            .limit(1);
    ...
    static async openShift(userId, initialCash) {
    ...
    static async getShiftSummary(shiftId) {
        ...
        */
        ...
        const cashFlow = ledgerEntriesWithRefs.reduce((acc, row) => {
            ...
            if (entry.type === 'sale' && row.saleDeleted)
                return acc;
            ...
            if (entry.type === 'sale' && row.saleVoided)
                return acc;
            if (entry.type === 'refund' && row.saleVoided)
                return acc;
            // If it's an expense entry and the expense was reversed/deleted, IGNORE IT
            if (entry.type === 'expense' && row.expenseDeleted)
                return acc;
        ...
        const salesData = await db.select({
            count: sql `count(${schema.sales.id})`,
            totalAmount: sql `sum(${schema.sales.totalAmount})`,
            nonCashAmount: sql `sum(case when ${schema.sales.paymentMethod} != 'CASH' then ${schema.sales.totalAmount} else 0 end)`
        })
            .from(schema.sales)
            .where(and(eq(schema.sales.shiftId, shiftId), eq(schema.sales.status, 'PAID'), eq(schema.sales.isVoided, false), eq(schema.sales.isDeleted, false)));
        const itemsData = await db.select({
            totalItems: sql `sum(${schema.saleItems.quantity})`
        })
        ...
        const summary = {
            ...
            nonCashTransactions: await db.select({
                ...
                .from(schema.sales)
                .where(and(eq(schema.sales.shiftId, shiftId), ne(schema.sales.paymentMethod, 'CASH'), eq(schema.sales.status, 'PAID'), eq(schema.sales.isVoided, false), eq(schema.sales.isDeleted, false)))
        };
        return summary;
    }
    static async closeShift(shiftId, data) {
        ...
        const denomsArray = Array.isArray(denominations)
            ? denominations
    ...
    static async handoverShift(fromShiftId, toUserId, cashAmount, approvedBy1, approvedBy2) {
        // Verify fromShift
        const fromShiftArr = await db.select().from(schema.shifts).where(eq(schema.shifts.id, fromShiftId)).limit(1);
        if (fromShiftArr.length === 0)
            throw new Error('Shift asal tidak ditemukan.');
