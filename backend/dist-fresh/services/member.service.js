...
export class MemberService {
    static async getAllMembers(search = '') {
        if (search) {
            return await db.select().from(schema.members)
                .where(or(ilike(schema.members.name, `%${search}%`), ilike(schema.members.phone, `%${search}%`)));
        }
        return await db.select().from(schema.members);
    }
    ...
    static async createMember(data) {
        ...
        if (existing)
            throw new Error(`Member dengan No HP ${data.phone} sudah terdaftar.`);
    ...
    static async adjustPoints(id, delta, reason = 'Manual Adjustment') {
    ...
    static async calcPointsEarned(totalAmount) {
        const settings = await this.getSettings();
        return Math.floor(totalAmount / settings.ratio);
    }
    static async calcPointsValue(points) {
        const settings = await this.getSettings();
        // points → Rupiah
        return Math.floor(points * settings.value);
    }
    static calcLevel(points) {
        if (points >= 1000)
            return 'gold';
        if (points >= 300)
            return 'silver';
