...
export class ProductService {
    static async getAllProducts(outletId) {
        let recipesQuery = db.select().from(schema.recipes).where(eq(schema.recipes.isActive, true));
        if (outletId) {
            recipesQuery = db.select().from(schema.recipes).where(and(eq(schema.recipes.isActive, true), eq(schema.recipes.outletId, outletId)));
        }
    ...
    static async getProductPhoto(id, outletId) {
        let filters = [eq(schema.recipes.id, id)];
        if (outletId)
            filters.push(eq(schema.recipes.outletId, outletId));
    ...
    static async updateProduct(id, data) {
        ...
        const result = await db.transaction(async (tx) => {
            try {
                ...
                if (outletId)
                    selectFilters.push(eq(schema.recipes.outletId, outletId));
                ...
                if (priceStandStr !== null)
                    updatePayload.priceStand = priceStandStr;
        ...
        if (result?.oldImageToDelete) {
            ...
            deleteFromCloudinary(result.oldImageToDelete).catch(err => console.error('[Cloudinary Cleanup] Failed:', err));
        }
        return result;
    }
    static async deleteProduct(id, outletId) {
        let filters = [eq(schema.recipes.id, id)];
        if (outletId)
            filters.push(eq(schema.recipes.outletId, outletId));
