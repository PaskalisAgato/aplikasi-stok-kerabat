...
let recentRequests = []; // true = OK, false = Error
let safeModeUntil = 0;
...
export const monitorMiddleware = async (req, res, next) => {
    ...
    res.send = function (body) {
        if (body) {
            if (typeof body === 'string')
                outboundSize = Buffer.byteLength(body);
            else if (Buffer.isBuffer(body))
                outboundSize = body.length;
            else
                outboundSize = Buffer.byteLength(JSON.stringify(body));
        }
    ...
    res.on('finish', async () => {
        ...
        if (recentRequests.length > SAFE_MODE_WINDOW)
        ...
        if (isSignificant || shouldSample) {
            try {
                const user = req.user; // Better-auth user
