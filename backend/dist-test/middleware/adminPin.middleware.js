...
export const requireAdminPin = async (req, res, next) => {
    try {
        ...
        const admins = await db.select().from(schema.users)
            .where(and(eq(schema.users.pin, adminPin), inArray(schema.users.role, ['Admin', 'Owner', 'Supervisor']), eq(schema.users.isDeleted, false)))
            .limit(1);
