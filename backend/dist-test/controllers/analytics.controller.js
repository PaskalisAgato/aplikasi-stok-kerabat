...
export class AnalyticsController {
    ...
    static async getShiftReports(req, res) {
        try {
    ...
    static async getDeadMenus(req, res) {
        try {
            const { startDate, endDate, outletId } = req.query;
            const start = new Date(`${startDate}T00:00:00+07:00`);
            const end = new Date(`${endDate}T23:59:59.999+07:00`);
            const data = await AnalyticsService.getDeadMenus({ start, end }, outletId ? parseInt(outletId) : undefined);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
            console.error(error);
    ...
    static async getInventoryVariance(req, res) {
        try {
            const { startDate, endDate, outletId } = req.query;
            const start = new Date(`${startDate}T00:00:00+07:00`);
            const end = new Date(`${endDate}T23:59:59.999+07:00`);
            const data = await AnalyticsService.getInventoryVariance({ start, end }, outletId ? parseInt(outletId) : undefined);
            res.status(200).json({ success: true, data });
        }
        catch (error) {
            console.error(error);
    ...
    static async getCrossOutletSummary(req, res) {
        try {
