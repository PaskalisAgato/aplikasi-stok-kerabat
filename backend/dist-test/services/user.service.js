...
export class UserService {
    static async loginByPin(role, pin) {
        ...
        const [user] = await db.select()
            .from(users)
            .where(and(inArray(users.role, allowedRoles), eq(users.pin, pin), eq(users.isDeleted, false)))
            .limit(1);
        return user;
    }
    ...
    static async verifyAdminPin(pin) {
        const [user] = await db.select()
            .from(users)
            .where(and(eq(users.pin, pin), inArray(users.role, ['Admin', 'Owner', 'Supervisor']), eq(users.isDeleted, false)))
            .limit(1);
        return user || null;
    }
