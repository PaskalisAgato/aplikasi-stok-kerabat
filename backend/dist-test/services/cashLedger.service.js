...
export class CashLedgerService {
    ...
    static async getSummary(shiftId) {
        const results = await db.select({
            type: schema.cashLedger.type,
            total: sql `sum(cast(${schema.cashLedger.amount} as decimal))`
        })
            ...
            .where(and(eq(schema.cashLedger.shiftId, shiftId), 
        // Filter: Ignore if its a sale/refund/expense that has been deleted
        or(
        // 1. It's not a sale or expense (e.g. adjustment, handover)
        and(ne(schema.cashLedger.type, 'sale'), ne(schema.cashLedger.type, 'refund'), ne(schema.cashLedger.type, 'expense')), 
        // 2. It's a sale that is NOT deleted AND NOT voided
        and(eq(schema.cashLedger.type, 'sale'), eq(schema.sales.isDeleted, false), eq(schema.sales.isVoided, false)), 
        // 3. It's an expense that is NOT deleted
        and(eq(schema.cashLedger.type, 'expense'), eq(schema.expenses.isDeleted, false)), 
        // 4. Default safetynet if no reference found at all
        and(isNull(schema.sales.id), isNull(schema.expenses.id)))))
