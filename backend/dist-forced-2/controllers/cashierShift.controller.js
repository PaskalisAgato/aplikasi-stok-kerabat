...
export class CashierShiftController {
    ...
    static async getAllActiveShifts(req, res) {
        try {
    ...
    static async openShift(req, res) {
        try {
            ...
            if (initialCash === undefined)
                return res.status(400).json({ error: 'Initial cash is required' });
    ...
    static async handoverShift(req, res) {
        try {
            const userId = req.user?.id;
            ...
            // 3. Perform Handover
            const result = await CashierShiftService.handoverShift(Number(currentShiftId), nextCashier.id, parseFloat(cashAmount), userId, // Approved by 1: Outgoing Cashier
