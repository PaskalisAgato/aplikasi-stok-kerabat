...
export class DiscountService {
    ...
    static async createDiscount(data, userId) {
    ...
    static async updateDiscount(id, data, userId) {
        const updateData = {};
        ...
        if (data.value !== undefined)
            updateData.value = data.value?.toString() || '0';
        if (data.conditions !== undefined)
            updateData.conditions = JSON.stringify(data.conditions);
        if (data.isActive !== undefined)
            updateData.isActive = data.isActive;
        if (data.isStackable !== undefined)
            updateData.isStackable = data.isStackable;
        if (data.isExclusive !== undefined)
            updateData.isExclusive = data.isExclusive;
        if (data.minPurchase !== undefined)
            updateData.minPurchase = data.minPurchase?.toString() || '0';
        if (data.startDate !== undefined)
            updateData.startDate = data.startDate ? new Date(data.startDate) : null;
        if (data.endDate !== undefined)
            updateData.endDate = data.endDate ? new Date(data.endDate) : null;
        // Financial Controls
        if (data.discountCap !== undefined)
            updateData.discountCap = data.discountCap ? data.discountCap.toString() : null;
        if (data.budgetLimit !== undefined)
            updateData.budgetLimit = data.budgetLimit ? data.budgetLimit.toString() : null;
        // Quota
        if (data.totalQuota !== undefined)
            updateData.totalQuota = data.totalQuota ? parseInt(data.totalQuota) : null;
        if (data.limitPerUser !== undefined)
            updateData.limitPerUser = data.limitPerUser ? parseInt(data.limitPerUser) : null;
        // Priority & Distribution
        if (data.priority !== undefined)
            updateData.priority = parseInt(data.priority);
        if (data.voucherCode !== undefined)
            updateData.voucherCode = data.voucherCode ? data.voucherCode.toUpperCase().trim() : null;
        // Audit
        if (userId)
     ...
     */
    static async getDiscountStats(id) {
     ...
     */
    static async redeemDiscounts(discountIds, discountAmount) {
     ...
     */
    static async evaluateDiscounts(cartItems, memberLevel, memberId, voucherCode, orderSource) {
            ...
            switch (discount.type) {
                ...
                case 'bundling': {
                    if (!conditions.productIds || !Array.isArray(conditions.productIds))
                        break;
                    const targetIds = conditions.productIds.map(Number);
                    const hasAll = targetIds.every((tid) => itemsWithMetadata.some(i => i.recipeId === tid));
                    console.log(`[PROMO BUNDLE] "${discount.name}" -> target: ${targetIds} | hasAll: ${hasAll} | cart: ${itemsWithMetadata.map(i => i.recipeId)}`);
                    if (hasAll) {
                        // How many full sets?
                        const setQty = Math.min(...targetIds.map((tid) => {
                        ...
                        const bundleBaseSetPrice = targetIds.reduce((sum, tid) => {
            ...
            try {
                ...
                if (vResult.valid) {
                    ...
                    const [qrTemplate] = await db.select()
                        ...
                        .where(and(eq(schema.discounts.type, 'qr_voucher'), eq(schema.discounts.isActive, true)))
                        .limit(1);
