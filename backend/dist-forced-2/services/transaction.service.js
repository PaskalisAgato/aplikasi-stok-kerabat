...
export class TransactionService {
    ...
    static async getAllTransactions(limit = 100, offset = 0, startDate, endDate) {
    ...
    static async processCheckout(data, userId) {
        ...
        for (const item of items) {
            ...
            if (freshPrice === undefined)
                throw new Error(`Menu dengan ID ${item.recipeId} tidak ditemukan`);
        ...
        return await db.transaction(async (tx) => {
            ...
            if (sourceId) {
                ...
                if (!existingSale)
                    throw new Error('Bill asal tidak ditemukan');
                ...
                if (oldItems.length > 0) {
                    const oldRecipeIds = oldItems.map((i) => i.recipeId);
                    ...
                    const oldInvIds = [...new Set(oldBoms.map((b) => b.inventoryId))];
                    let oldInvMap = new Map();
                    if (oldInvIds.length > 0) {
                        ...
                        oldInvMap = new Map(oldInvItems.map((i) => [i.id, i.unit]));
                    }
                    const revertMap = new Map();
                    for (const item of oldItems) {
                        const boms = oldBoms.filter((b) => b.recipeId === item.recipeId);
                        for (const b of boms) {
            ...
            // Log to Audit
            console.log("DEBUG: Processing Audit Log for Transaction", { finalizedSaleId, outletId });
            ...
            if (saleValues.status === 'PAID' && data.voucherCode && data.voucherCode.startsWith('KKT-')) {
                try {
                    ...
                    await VoucherService.redeemVoucher(data.voucherCode, activeShift?.outletId || 1, finalizedSaleId);
                }
                catch (vRedeemErr) {
            ...
            KdsService.notifyNewOrder(activeShift?.outletId || 1, {
                ...
                items: items.map((i) => ({
                    recipeId: i.recipeId,
                    name: dbRecipes.find((r) => r.id === i.recipeId)?.name || 'Menu',
                    quantity: i.quantity,
                    notes: i.notes
                })),
                status: data.status || 'PAID',
    ...
    static async deleteTransaction(saleId, adminId, adminPin) {
    ...
    static async voidTransaction(id, reason, userId, adminPin) {
    ...
    static async exportTransactionsExcel(startDate, endDate) {
        // A. Fetch Data
        const filters = [eq(schema.sales.isDeleted, false)];
        if (startDate)
            filters.push(gte(schema.sales.createdAt, new Date(startDate)));
        ...
        _sales.filter(s => !s.isVoided).forEach(sale => {
            ...
            if (sale.paymentMethod === 'CASH')
                dailyAgg[dayKey].cash += parseFloat(sale.totalAmount);
            else if (sale.paymentMethod === 'QRIS')
                dailyAgg[dayKey].qris += parseFloat(sale.totalAmount);
            else
                dailyAgg[dayKey].other += parseFloat(sale.totalAmount);
            const items = _items.filter(i => i.saleId === sale.id);
