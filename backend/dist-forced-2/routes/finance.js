...
financeRouter.get('/expenses', async (req, res) => {
    try {
        ...
        if (qOutletId) {
            filters.push(eq(schema.expenses.outletId, parseInt(qOutletId)));
        }
        if (startDate) {
        ...
        const summaryResult = await db.select({
            totalAmount: sql `COALESCE(SUM(CAST(${schema.expenses.amount} AS DECIMAL)), 0)`,
            count: sql `COUNT(*)`
        })
            .from(schema.expenses)
            .where(whereClause);
...
financeRouter.get('/reports', requireAdmin, async (req, res) => {
    try {
        ...
        if (outletId)
            baseFilters.push(eq(schema.sales.outletId, outletId));
        ...
        if (outletId)
            todayFilters.push(eq(schema.sales.outletId, outletId));
        ...
        if (outletId)
            expenseFilters.push(eq(schema.expenses.outletId, outletId));
...
financeRouter.get('/hpp', requireAdmin, async (req, res) => {
    try {
        ...
        const hppResult = await db.select({
            totalHPP: sql `COALESCE(SUM(CAST(${schema.saleItems.quantity} AS float) * CAST(${schema.recipeIngredients.quantity} AS float) * CAST(${schema.inventory.pricePerUnit} AS float)), 0)`
        })
            .from(schema.saleItems)
            ...
            .innerJoin(schema.inventory, eq(schema.recipeIngredients.inventoryId, schema.inventory.id))
            .where(and(gte(schema.sales.createdAt, thirtyDaysAgo), eq(schema.sales.isDeleted, false), eq(schema.sales.isVoided, false)));
        ...
        const ingredientsHPP = await db.select({
            ...
            totalCost: sql `SUM(CAST(${schema.saleItems.quantity} AS float) * CAST(${schema.recipeIngredients.quantity} AS float) * CAST(${schema.inventory.pricePerUnit} AS float))`,
            totalQty: sql `SUM(CAST(${schema.saleItems.quantity} AS float) * CAST(${schema.recipeIngredients.quantity} AS float))`
        })
            .from(schema.saleItems)
            .innerJoin(schema.sales, eq(schema.saleItems.saleId, schema.sales.id))
            .innerJoin(schema.recipeIngredients, eq(schema.saleItems.recipeId, schema.recipeIngredients.recipeId))
...
financeRouter.get('/cogs', async (req, res) => {
    try {
        ...
        const currentMonthProcurement = await db.select({
            totalAmount: sql `COALESCE(SUM(CAST(${schema.stockMovements.quantity} AS float) * CAST(${schema.inventory.pricePerUnit} AS float)), 0)`
        })
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(and(eq(schema.stockMovements.type, 'IN'), gte(schema.stockMovements.createdAt, currentMonthStart)));
        ...
        const lastMonthProcurement = await db.select({
            ...
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(and(eq(schema.stockMovements.type, 'IN'), gte(schema.stockMovements.createdAt, lastMonthStart), lte(schema.stockMovements.createdAt, currentMonthStart)));
        ...
        const topExpenditureItems = await db.select({
            ...
            totalCost: sql `SUM(CAST(${schema.stockMovements.quantity} AS float) * CAST(${schema.inventory.pricePerUnit} AS float))`,
            totalQty: sql `SUM(CAST(${schema.stockMovements.quantity} AS float))`
        })
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(and(eq(schema.stockMovements.type, 'IN'), gte(schema.stockMovements.createdAt, currentMonthStart)))
...
financeRouter.get('/profit-loss', requireAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        ...
        if (startDate) {
            ...
            if (!isNaN(d.getTime()))
                salesFilters.push(gte(schema.sales.createdAt, d));
        }
        if (endDate) {
            const d = new Date(`${endDate}T23:59:59.999+07:00`);
            if (!isNaN(d.getTime()))
                salesFilters.push(lte(schema.sales.createdAt, d));
        }
        ...
        if (startDate) {
            const d = new Date(`${startDate}T00:00:00+07:00`);
            if (!isNaN(d.getTime()))
                expenseFilters.push(gte(schema.expenses.expenseDate, d));
        }
        if (endDate) {
            const d = new Date(`${endDate}T23:59:59.999+07:00`);
            if (!isNaN(d.getTime()))
                expenseFilters.push(lte(schema.expenses.expenseDate, d));
        }
        ...
        const breakdownRaw = await db.select({
            ...
            name: schema.recipes.name,
            totalSold: sql `SUM(CAST(${schema.saleItems.quantity} AS float))`,
            revenue: sql `SUM(CAST(${schema.saleItems.subtotal} AS float))`,
            totalHPP: sql `SUM(CAST(${schema.saleItems.quantity} AS float) * CAST(${schema.recipes.costPrice} AS float))`
        })
            .from(schema.saleItems)
            .innerJoin(schema.sales, eq(schema.saleItems.saleId, schema.sales.id))
            .innerJoin(schema.recipes, eq(schema.saleItems.recipeId, schema.recipes.id))
...
financeRouter.get('/waste-analysis', requireAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        ...
        if (startDate) {
            const d = new Date(`${startDate}T00:00:00+07:00`);
            if (!isNaN(d.getTime()))
                dateFilters.push(gte(schema.stockMovements.createdAt, d));
        }
        if (endDate) {
            const d = new Date(`${endDate}T23:59:59.999+07:00`);
            if (!isNaN(d.getTime()))
                dateFilters.push(lte(schema.stockMovements.createdAt, d));
        }
        // --- 1. Total Used ---
        ...
        const usedResult = await db.select({
            totalUsed: sql `COALESCE(SUM(CAST(${schema.stockMovements.quantity} AS float)), 0)`
        })
            .from(schema.stockMovements)
            .where(outFilter);
        ...
        const wasteRaw = await db.select({
            ...
            wasteQty: sql `SUM(CAST(${schema.stockMovements.quantity} AS float))`,
            wasteCost: sql `SUM(CAST(${schema.stockMovements.quantity} AS float) * CAST(${schema.inventory.pricePerUnit} AS float))`
        })
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(wasteFilter)
...
financeRouter.get('/owner-income', requireAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
        ...
        if (startDate) {
            const d = new Date(`${startDate}T00:00:00+07:00`);
            if (!isNaN(d.getTime()))
                filters.push(gte(schema.ownerIncome.incomeDate, d));
        }
        if (endDate) {
            const d = new Date(`${endDate}T23:59:59.999+07:00`);
            if (!isNaN(d.getTime()))
                filters.push(lte(schema.ownerIncome.incomeDate, d));
        }
        const whereClause = and(...filters);
        ...
        const summaryResult = await db.select({
            totalAmount: sql `COALESCE(SUM(CAST(${schema.ownerIncome.amount} AS DECIMAL)), 0)`,
            count: sql `COUNT(*)`
        })
            .from(schema.ownerIncome)
            .where(whereClause);
...
financeRouter.post('/owner-income', requireAdmin, async (req, res) => {
    try {
...
financeRouter.delete('/owner-income/:id', requireAdmin, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
        if (isNaN(id))
            return res.status(400).json({ success: false, message: 'ID tidak valid' });
        const [existing] = await db.select()
            ...
            .limit(1);
        if (!existing)
            return res.status(404).json({ success: false, message: 'Data uang masuk tidak ditemukan' });
