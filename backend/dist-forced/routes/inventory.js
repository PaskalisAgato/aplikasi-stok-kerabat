...
import { eq, sql, and, gte, desc, ilike, inArray } from 'drizzle-orm';
...
inventoryRouter.get('/export', async (req, res) => {
    try {
        const outletId = req.query.outletId ? parseInt(req.query.outletId) : 1;
...
inventoryRouter.get('/:id', async (req, res) => {
    try {
        ...
        const [item] = await db.select({
            ...
            .from(schema.inventory)
            .where(and(eq(schema.inventory.id, id), eq(schema.inventory.isDeleted, false)))
            .limit(1);
        if (!item)
...
inventoryRouter.get('/:id/price-logs', requireAuth, async (req, res) => {
    try {
        const id = parseInt(req.params.id);
...
inventoryRouter.get('/waste/summary', async (req, res) => {
    try {
        ...
        const movements = await db.select({
            ...
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(and(eq(schema.stockMovements.type, 'WASTE'), gte(schema.stockMovements.createdAt, thirtyDaysAgo), eq(schema.inventory.outletId, outletId)));
...
inventoryRouter.get('/waste/history', async (req, res) => {
    try {
        ...
        const wasteLogs = await db.select({
            ...
            .from(schema.stockMovements)
            .innerJoin(schema.inventory, eq(schema.stockMovements.inventoryId, schema.inventory.id))
            .where(inArray(schema.stockMovements.type, ['WASTE', 'OPNAME_WASTE']))
            .orderBy(desc(schema.stockMovements.createdAt))
            .limit(Number(limit))
...
inventoryRouter.post('/opname', requireAuth, async (req, res) => {
    try {
        ...
        await db.transaction(async (tx) => {
            for (const adj of adjustments) {
                ...
                if (delta === 0 && containerWeight === undefined)
                    continue;
    ...
    try {
        ...
        const results = await db.transaction(async (tx) => {
            ...
            if (oldItem.isDeleted)
                return { error: 'Item has been deleted', status: 410 };
...
inventoryRouter.post('/recovery/sanitize-nan', requireAdmin, async (req, res) => {
    try {
